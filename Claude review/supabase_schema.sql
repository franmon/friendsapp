...
-- ============================================================
-- FUNCIÓN: unirse a un grupo por código de invitación
-- Necesaria porque la policy de SELECT sobre `groups` solo deja ver
-- grupos de los que ya eres miembro; un usuario nuevo no podría
-- localizar el grupo por su código. SECURITY DEFINER salta RLS de
-- forma controlada: solo expone el id del grupo y crea la membresía.
-- ============================================================
...
BEGIN
  ...
  -- Inserta la membresía; si ya existe, no falla (idempotente)
